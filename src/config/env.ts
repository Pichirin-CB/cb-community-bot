...
const envSchema = z.object({
  ...
  DATABASE_PATH: z.string().default("./data/cb_community.sqlite"),
  ...
  ENABLE_SYSTEM_MONITOR: z
    .string()
    .default("false")
    .transform((value) => ["1", "true", "yes", "on"].includes(value.toLowerCase())),
  BOT_PRESENCE: z.string().default("Con la comunidad 🎮"),
...
export const env = {
  ...
  LOG_FILE: path.resolve(projectRoot, "logs", "cb_community.log"),
};
...
export function requireRuntimeSecrets(): void {
  ...
  if (!env.DISCORD_GUILD_ID && env.COMMAND_DEPLOYMENT_MODE === "guild") {
